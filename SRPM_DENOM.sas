...
*   Date      Comment                                                         *;
*   ========================================================================= *;
*   20170131  Initial GitHub version finalized.                               *;
*******************************************************************************;

*******************************************************************************;
* INITIAL PROGRAM SETUP:                                                      *;
*   - %include the full path to your local StdVars file.                      *;
*   - %let root = location to which you extracted contents of zip file.       *;
...
data _null_;
  x = index("&root", "SRPM_DENOM");
  if x = 0 then call symput('root', strip("&root" || "/SRPM_DENOM"));
run;
...
proc printto log="&root/LOCAL/SRPM_DENOM_&_SITEABBR..log" new;
run;
...
proc sql;
  ...
  select 
    ...
    , dx.dx
    /**************************************************************************
    * BEGIN LOCAL CODE: Store available Clarity PAT_ENC_CSN_IDs (PECIs) from  *
    * VDW Encounter, Px, and/or Dx tables.                                    *
    **************************************************************************/
    , case
        when enc.pat_enc_csn_id > 0 then enc.pat_enc_csn_id 
        else . 
      end as enc_peci
    , case
        when px.pat_enc_csn_id > 0 then px.pat_enc_csn_id 
        else .
      end as px_peci
    , case
        when dx.pat_enc_csn_id > 0 then dx.pat_enc_csn_id
        else .
      end as dx_peci
    /* END LOCAL CODE */
...
ods pdf file="&root/LOCAL/SRPM_DENOM_SUM_&_siteabbr..pdf" notoc
  style=minimal
;

proc print data=mh_ute_freq noobs label;
  title1 "&_siteabbr SRS2 Denominator v&v: VDW-based population of internal,
 ...
 &start and 30JUN2015 among patients aged 13+ years"
  ...
  sum count percent;
  footnote "Filename: &root/LOCAL/SRPM_DENOM_SUM_&_siteabbr..pdf";
run;
...
*******************************************************************************;
%dsdelete(ret.SRS2_DENOM_SUM_&_siteabbr);

data ret.SRS2_DENOM_SUM_&_siteabbr;
  set mh_ute_freq;
...
%dsdelete(loc.SRS2_DENOM_FULL_&_siteabbr);

data loc.SRS2_DENOM_FULL_&_siteabbr;
